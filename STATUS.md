...
**Next:** decide whether shipping the skill warrants `v0.5.0` — it adds a file users install, so the release checklist would apply.
**Unreleased on main:** `skills/claude-session/`, the Agent Skill that drives these commands from plain language. It used to live only in the maintainer's private config backup, which was the wrong home: the skill documents this tool's commands and destructive-command rules, so it belongs beside them and versions with them. Absolute local paths were stripped from it on the way in, since this repository is public.
**Known issues:**

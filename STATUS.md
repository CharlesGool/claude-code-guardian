...
**Snapshots:** none yet
**In progress:** implementation complete and installed live on this host (`systemctl is-active claude-guardian` → active), launching `claude --permission-mode auto --remote-control` by default with an unattended-only keepalive (double-Enter nudge + `/remote-control` refresh) so the session stays remotely controllable through long idle stretches and survives a first-run onboarding/trust screen. Added `claude-guardian purge` for a genuinely complete teardown (session, socket, config, binary), distinct from the deliberately-conservative `uninstall`. Real end-to-end verification against the actual `claude` binary and a real second-host deployment found and fixed several bugs (see DECISIONS.md): `StartLimitIntervalSec` unit-section placement, `KillMode` killing the live session on restart, unattended timers firing immediately instead of waiting out their interval, `CLAUDE_BIN` not resolving under systemd's minimal PATH, and a stale hardcoded line range in `usage()`. The auto-Enter mechanism was flagged live by Claude Code's own auto-mode classifier as defeating the human-in-the-loop fallback; user was shown the tradeoff explicitly and confirmed proceeding (see DECISIONS.md 2026-08-16).
...
**Known issues:** none currently open. A real second-host deployment failed (service went `failed` — `claude` not on systemd's PATH, only on the interactive shell's) and was fixed: `write_default_config` now bakes an absolute `CLAUDE_BIN` path at install time, and `preflight_enforce`/`preflight_report` self-heal existing configs via a `bash -ic` fallback lookup. Both fixed paths verified with functional tests (see DECISIONS.md). Reboot-survival not yet independently verified on this host (would require an actual reboot; `systemctl is-enabled` was confirmed instead). Whether an idle-but-network-healthy Remote Control session can go stale on its own (vs. only after real connectivity loss) is undocumented upstream — `REMOTE_CONTROL_REFRESH_SEC` refreshes proactively regardless, so this is a residual unknown rather than a blocking gap.
**Blocked on:** nothing.

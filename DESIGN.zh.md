...
- **无人值守保活机制**（新增，见 `DECISIONS.md` 2026-08-16 "始终可远程控制"）：当 `tmux list-clients` 显示无人接入时，循环会定期 (a) 发送一个裸 Enter，清除 `--permission-mode auto` 在多次被分类器拦截后可能回退出现的确认弹窗；(b) 重新执行 `/remote-control` 刷新连接，抢在 Anthropic 文档所述的约 30 分钟"无法连接到 Remote Control 服务器"阈值被真正触发之前。这两者一旦有客户端接入就立刻停止（每个 tick 都会检查）。
- 这两层是刻意独立的：`systemctl stop claude-guardian` 只停止*监督循环*本身，不会杀掉存活的 tmux 会话，因此一次日常运维操作不会打断正在对话中的操作者。彻底拆除是一个单独的、显式的步骤（见 README 里的 `uninstall`）。
...
- **仓库在挂载到 `/root/MyGithub_Project` 这个运维用 NAS 上时，所在的 CIFS 文件系统挂载参数固定为 `file_mode=0644`**——对 `bin/claude-guardian.sh` 执行 `chmod +x` 在那里是静默无效的（退出码 0，权限位没有变化）。可执行位改为在提交时直接写进 git 树：`git update-index --chmod=+x bin/claude-guardian.sh`，所以在其他任何文件系统上正常 `git clone` 出来的文件会是可执行的。在这个 NAS 挂载的工作副本里直接操作时，请显式用 `bash bin/claude-guardian.sh ...` 而不是 `./bin/claude-guardian.sh`。

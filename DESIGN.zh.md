...
> 译自 `DESIGN.md`（v0.2.0）。如有冲突，以英文版为准。
...
- 让每个实例的远程控制链接都能在不接管它的情况下取到——创建时自动捕获一次，之后每次无人值守的刷新周期再捕获一次，按实例存下来，由 `list`/`url` 打印。这样做的意义在于：操作者可以完全从另一个驱动这个工具 CLI 的 Claude Code 会话里创建、发现、接入一个会话，永远不需要自己的终端。
- 让实例可以被**暂停**（`deactivate`/`activate`：只停/恢复监督，tmux 会话继续跑），这与被**归档**（`archive`/`resume`：保存滚动记录和对话 id，再杀掉进程；之后用 `claude --resume` 重建）是两件刻意区分开的、影响范围完全不同的操作。
- 扛得住重启（每个被启用的实例的 service 都会开机自启）。
- 扛得住 `claude` 进程本身被杀——Ctrl+C、崩溃、`exit`、OOM kill——在几秒内自动重启它，且不丢失周围的会话。
- 在长时间无人值守的情况下依然保持真正可达，而不只是"进程还在跑"：定期清除 `auto` 权限模式可能回退出现的确认弹窗，并在 Remote Control 连接变得不新鲜之前主动刷新它（这里涉及的安全权衡见 Known limitations）。
...
```
                     某个实例的监督进程重启 / 崩溃，或宿主机重启
                                    |
                                    v
   systemd (Restart=always) ---> claude-guardian run <name>  （前台循环）
   claude-guardian@<name>.service    |
   （每个 name 对应一个实例，          | 每隔 CHECK_INTERVAL_SEC：
    来自同一个模板 unit）              | tmux 会话是否存在？pane 是否已死？是否有客户端接管？
                                      | （若无人值守：敲 Enter / 刷新
                                      |  /remote-control 并重新捕获链接——
                                      |  各自按更长的间隔单独触发，见下文）
                                      v
                     tmux 会话 "<name>"（remain-on-exit on）
                     ——可能是若干个会话中的一个，全都跑在
                       同一个 tmux 服务器 / $TMUX_SOCKET 上
                                    |
                                    v
        claude --permission-mode auto --remote-control --session-id <uuid>
        （如果这个实例是通过 `claude-guardian resume <archive-id>` 创建的，
         则用 --resume <uuid> 取代 --session-id）
                                    ^                       ^
                                    |                       |
                       操作者：ssh + `claude-guardian        操作者：claude.ai
                       attach <name>`（tmux attach）          网页/手机（Remote Control）——
                                                               链接从 pane 里抓取并存下来，
                                                               所以 `claude-guardian
                                                               url <name>` 不用接管就能
                                                               打印出来
...
- **无人值守保活**（见 `DECISIONS.md` 2026-08-16 "always remotely controllable"）：当 `tmux list-clients` 显示某个实例的会话无人接管时，它的循环会定期地（a）发送 Enter——间隔一秒发两次——清除 `--permission-mode auto` 在多次被分类器拦截后可能回退出现的确认弹窗；以及（b）重新执行 `/remote-control`，一方面在 Anthropic 文档所述的约 30 分钟"无法连接 Remote Control 服务器"阈值到来之前刷新连接，另一方面重新抓取（可能已经变化的）`claude.ai/code/...` 链接存进实例状态。只要有客户端接管，两者都会立即停止（每个 tick 都会检查一次）。`create_session` 在启动 `claude` 之后也会发送同样的双击 Enter，原因相同：一次真正的首次运行可能会出现需要两次 Enter 才能清除的引导/信任屏幕，不应该非等到第一次无人值守敲击的间隔才处理——之后它还会自己同步做一次 `/remote-control` 捕获，这样 `new` 就能立刻打印出链接，不用等到 `REMOTE_CONTROL_REFRESH_SEC` 之后的第一次周期性刷新。如果 `claude` 此时已经在正常的提示符上，多发一次 Enter 也只是一次无害的空操作，所以与其去检测到底显示的是哪个屏幕，不如始终发两次更划算。
...
- **归档 / 恢复**：`archive <name>` 先停止监督，把完整的滚动记录（`tmux capture-pane -pS -`）和该实例的 `claude_session_id` 保存到 `/var/lib/claude-guardian/archive/<name>-<ts>/`，然后杀掉 tmux 会话——这是一个刻意的、默认需要确认的破坏性操作（见 `DECISIONS.md`，2026-08-17，"archive kills the process"）。`resume <archive-id> [new-name]` 会带着设好的 `RESUME_SESSION_ID` 重建一个实例，使得 `create_session` 传的是 `--resume <uuid>` 而不是重新铸造一个新的——操作者就此接回同一段对话。
...
| 实现语言 | 类 POSIX 的 Bash | bash（Debian 默认的 `/bin/bash`） | 整个工具就是进程编排，外加对 `tmux`/`systemd`/`apt-get` 的调用；引入一个脚本运行时不会带来任何好处，只会多一个依赖。 |
...
全局变量放在 `/etc/claude-guardian/config.env`，是一个纯粹的 `KEY="value"` shell 文件，最先被 source。按实例的覆盖项（`WORKDIR`、`CLAUDE_ARGS`、`CLAUDE_BIN`）放在 `/etc/claude-guardian/instances/<name>.env`，只对该实例在其之上再 source 一次——见 `new --workdir`/`--args`/`--claude-bin`。仓库里的 `.env.example` 作为参考记录了同一份全局默认值（本项目没有单独的应用层 `.env`——安装后的那份配置文件本身就是运行时配置）。
...
| `REMOTE_CONTROL_REFRESH_SEC` | 仅无人值守时生效：无 tmux 客户端接管这么多秒后，重新执行 `/remote-control` 刷新连接并重新捕获链接；`0` 表示禁用 | `1200` | 全局 | 否 |
...
5. `claude-guardian attach`——验证：把你带进 tmux 里一个存活的 `claude` 终端（名字默认是 `claude-code`），且 pane 里会显示一行 `/remote-control is active ... https://claude.ai/code/session_...`——这个链接可以脱离这次 SSH 连接、独立地在网页或手机上控制，也可以不接管、直接用 `claude-guardian url claude-code` 打印出来。用 tmux 前缀键（默认 `Ctrl+b`）接 `d` 分离——**不要**用 Ctrl+C。
...
11. 从 `claude-code` 分离，放着无人值守超过 `UNATTENDED_NUDGE_SEC`——验证：`claude-guardian logs claude-code` 会在那个时间点打印一行 `sending Enter in case a prompt is stuck`，之后（`REMOTE_CONTROL_REFRESH_SEC` 之后）打印一行 `refreshing remote control connection`，且重新接管再分离一次之后，两者都不会立即再次触发（计时器在接管时会重置）。
...
- **捕获远程控制链接的过程会往 tmux pane 里发送真实的按键**（`C-u`、`/remote-control`、`Enter`）——发生在实例创建时（紧跟在引导阶段的双击 Enter 之后）以及每次无人值守的刷新周期。如果 `claude` 恰好意外地还停留在正常提示符以外的某个屏幕上（比如引导流程超出了已经发送的那两次 Enter），这些按键就可能被敲进错误的地方——无害（这一步本身不会自动确认任何东西），但可能留下需要手动用 `claude-guardian attach <name>` 清理的杂散文本。这和双击 Enter 清除引导屏幕、以及无人值守敲击是同一类"盲发按键"的权衡，参见上面 `UNATTENDED_NUDGE_SEC` 那条。
...
- **`install` 会原地迁移一个 v0.1.0 的单实例部署**（`migrate_legacy_unit`）：它会禁用/移除旧的 `claude-guardian.service`，并针对*同一个* tmux 会话启用新的 `claude-guardian@claude-code.service`。这依赖新旧两个 unit 里都设了 `KillMode=process`（上面已验证过）——如果未来某次改动不小心把这个设置去掉了，这条迁移路径就需要针对一个存活会话重新验证一遍，才能再信任它。已在一台真实的生产主机上验证过：`install` 执行前后，tmux pane 的 PID 和存活状态都没有变化。
- **一个被迁移过来的既有会话，在真正被重建之前，永远拿不到被跟踪的 `claude_session_id`、`workdir` 或已捕获的链接**——这是在上面那次生产迁移中发现的。`respawn-pane` 只会重放 tmux 保存的*最初那条*会话创建命令；对于一个早于 v0.2.0 就存在的会话来说，那条原始命令里没有 `--session-id`，所以任何一次崩溃重生都不会补上这个字段。实际影响：`list` 会把这类实例的 workdir 显示为 `-`，URL 显示"尚未捕获"，直到周期性的无人值守 `/remote-control` 刷新最终捕获到链接为止（状态并没有被卡住，只是来得晚）；而对这个特定实例执行 `archive` 会保存一个空的 `claude_session_id`——之后 `resume` 会正确地拒绝执行，转而指向原始滚动记录，而不是瞎猜。想让一个被迁移的实例拿到真正可恢复的会话 id，唯一的办法是让它的 tmux 会话真正结束并被重建（比如刻意 `archive` 它一次，再 `resume`/`new` 回来）——没有原地回填的办法，手工编辑它的状态文件也只会伪造一个 `claude` 从未真正用过的 id。
- **分离要用 tmux 前缀键，不要用 Ctrl+C——而且单次 Ctrl+C 本来也杀不死 `claude`。** 已对真实 CLI 验证过：Claude Code 把单次 Ctrl+C 当作"中断当前这一轮"（和大多数 REPL 一样），不是退出——pane 依然存活，什么都不会被重新拉起。要连按两次 Ctrl+C（或者 `/exit`）才会真正终止进程，此时 pane 才会死掉、guardian 才会把它重新拉起来，这符合最初的要求：一次刻意的杀掉动作绝不能导致零个存活实例。不管要按几次 Ctrl+C 才能退出，这都不是一种干净的离开会话的方式——请始终使用 tmux 前缀键 + `d`。
...
- **`REMOTE_CONTROL_REFRESH_SEC` 默认的 1200 秒是基于已有文档做出的一个主动的估计值，不是精确复现。** Anthropic 的文档说一个"约 30 分钟内无法连接 Remote Control 服务器"的会话需要手动执行一次 `/remote-control` 才能重连；而一个空闲但网络正常的会话是否也会按照自己的时间线变得不新鲜，在为这个功能做调研时被明确标记为文档未覆盖的情况。无论哪种情况，在 20 分钟就刷新一次都稳稳地落在文档所述 30 分钟失效窗口之内。

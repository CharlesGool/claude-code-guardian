...
- [ ] 2026-08-21 commit the isolated test suite into the repo — it has been written and thrown away twice now (the 23-case Remote Control suite died with the v0.7.0 withdrawal, the 48-case boot-floor suite lives only in a scratch directory), so a regression has nothing to fail against; start by adding `tests/` plus a one-line runner to README
